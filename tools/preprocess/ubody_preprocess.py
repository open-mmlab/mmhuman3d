...
def process_vid(vid):
    ...
    idxs_anno = [
        idx for idx, anno in enumerate(anno_param['annotations'])
        if int(anno['id']) in ids
    ]

    for idx in tqdm(
            idxs_anno, desc=f'Video frams: {seq}', leave=False, position=2):
        kp_param = anno_param['annotations'][idx]
        image_id = kp_param['image_id']

        image_info = anno_param['images'][image_id]

        # generate image info
        image_path = os.path.join(image_base_path, image_info['file_name'])
        id = kp_param['id']

        if str(image_id) not in smplx_param.keys():
            continue
