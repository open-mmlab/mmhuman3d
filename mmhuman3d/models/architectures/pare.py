...
class PARE(BaseArchitecture, metaclass=ABCMeta):
    ...
    Args:
        ...
        init_cfg (dict or list[dict], optional): Initialization config dict.
            Default: None.
        image_res (int, optional): image resolution for
            part silhouette rendering. Default: 224.
        focal_length (int, optional): focal length for
            part silhouette rendering. Default: 5000.
    """
    ...
    def __init__(
        ...
        K, R = get_default_camera(
            focal_length=self.focal_length, img_size=self.image_res)
    ...
    def get_smpl_result(self,
        ...
        if camera is not None:
            ...
            joints2d, cam_t = project_points(smpl_output.joints, camera,
                                             focal_length, img_res)
    ...
    def compute_segm_loss(self, pred_seg: torch.Tensor, gt_seg: torch.Tensor,
                          has_smpl: torch.Tensor):
        """Compute loss between part segmentation map and part attention
        map."""
        ...
        if ph != h or pw != w:
            pred_seg = F.interpolate(
                input=pred_seg, size=(h, w), mode='bilinear')
    ...
    def compute_losses(self, predictions: dict, targets: dict):
        ...
        pred_keypoints3d, _ = convert_kps(
            pred_keypoints3d, src=self.convention, dst=self.loss_convention)
        gt_keypoints3d, _ = convert_kps(
            gt_keypoints3d, src=self.convention, dst=self.loss_convention)
        gt_keypoints2d, _ = convert_kps(
            gt_keypoints2d, src=self.convention, dst=self.loss_convention)
        ...
        if self.loss_segm_mask is not None:
            losses['part_seg_loss'] = self.compute_segm_loss(
                pred_seg, gt_seg, has_smpl)
        return losses
