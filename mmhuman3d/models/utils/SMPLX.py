...
class SmplxFaceMergeFunc():
    ...
    def __call__(self, body_predictions, face_predictions):
        ...
        body_predictions['pred_param']['expression'] = face_predictions['pred_param']['expression'][:,:self.num_expression_coeffs]
        return body_predictions
...
def get_crop_info(points, img_metas, scale_factor: float = 1.0, crop_size: int = 256):
    ...
    crop_transforms = torch.cat([crop_transforms,
        torch.tensor([0.0,0.0,1.0],dtype = dtype, device=device).expand([batch_size,1,3])
        ], dim = 1)
...
class SmplxHandCropFunc():
    ...
    def build_hand_mean(self, global_orient, body_pose, betas, left_hand_pose,
        ...
        right_finger_pose_condition = (right_hand_pose if self.condition_hand_finger_pose else
            self.model_head.get_mean('right_hand_pose',batch_size = batch_size))
        right_hand_mean = torch.cat([right_wrist_pose, right_finger_pose_condition, shape_condition, camera_mean], dim=1)

        left_finger_pose_condition = (left_to_right_hand_pose if self.condition_hand_finger_pose else
            self.model_head.get_mean('right_hand_pose',batch_size = batch_size))
        # Should be Bx31
        left_hand_mean = torch.cat([left_to_right_wrist_pose, left_finger_pose_condition, shape_condition, camera_mean], dim=1)
...
class SmplxFaceCropFunc():
    ...
    def build_face_mean(self, global_orient, body_pose, betas, raw_jaw_pose, expression, batch_size):
        ...
        betas_condition = (
            ...
            if self.condition_face_shape else
            self.model_head.get_mean('shape', batch_size=batch_size)
        )
        ...
        expression_condition = (
            ...
            if self.condition_face_expression else
            self.model_head.get_mean('expression', batch_size=batch_size)
        )

        # Should be Bx(Head pose params)
        face_mean.append(torch.cat([neck_pose_condition, jaw_pose_condition, betas_condition, expression_condition,
                camera_mean.reshape(batch_size, -1),], dim=1))
