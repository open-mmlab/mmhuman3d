...
from mmhuman3d.utils.geometry import rot6d_to_rotmat_pare, get_coord_maps

from mmhuman3d.models.backbones.resnet import BasicBlock, conv1x1, conv3x3

from mmhuman3d.models.layers.coattention import CoAttention
from mmhuman3d.models.layers.softargmax import softargmax2d, get_heatmap_preds
from mmhuman3d.models.layers import LocallyConnected2d, KeypointAttention, interpolate
from mmhuman3d.models.layers.non_local import dot_product

from mmhuman3d.utils.keypoint_utils import get_smpl_neighbor_triplets
...
class PareHead(BaseModule):
    def __init__(
        ...
        if backbone.startswith('hrnet'):
            if use_resnet_conv_hrnet:
                ...
                # conv2d size unchanged, num_input_features is the output feature dim of the backbone
    ...
    def _get_pose_mlp(self, num_joints, output_size):
        if self.pose_mlp_num_layers == 1:
            ...
            # every position has different kernel
    ...
    def forward(self, features, gt_segm=None):
        ...
        pred_rotmat = rot6d_to_rotmat_pare(pred_pose).reshape(batch_size, 24, 3, 3)
