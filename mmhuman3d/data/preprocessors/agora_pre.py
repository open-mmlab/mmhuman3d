...
def agora_extract(dataset_path, out_path, mode='validation', fit='smplx'):

    # check accepted modes and fits
    accepted_modes = ['validation', 'train']
    if mode not in accepted_modes:
        raise ValueError('Input mode not in accepted modes. \
        ...
        for idx in range(len(df)):
            ...
            for pidx in valid_pers_idx:
                ...
                if fit == 'smplx':
                    ...
                    smplx['body_pose'].append(gt_smplx['body_pose'].reshape(
                        (21, 3)))
                    smplx['global_orient'].append(gt_smplx['global_orient'])
                    smplx['betas'].append(gt_smplx['betas'])
                    smplx['transl'].append(gt_smplx['transl'])
                    smplx['left_hand_pose'].append(gt_smplx['left_hand_pose'])
    ...
    if fit == 'smplx':
        ...
        smpl['body_pose'] = np.array(smpl['body_pose']).reshape((-1, 21, 3))
