...
def rotate_yaxis(R, t):
    """rotate the transformation matrix around z-axis by 180 degree ==>> let
    y-axis point up."""
    transform = np.eye(4)
    transform[:3, :3] = R
    transform[:3, 3] = t
...
class BehaveConverter(BaseModeConverter):
    ...
    def __init__(self, modes: List = []) -> None:
        ...
        self.misc_config = dict(
            ...
            smplh_source='original',
            image_size=(1536, 2048),
            fps=30,
            downsampled=True,
            downsampled_fps=1,
            keypoints3d='keypoints3d_original')
        self.smplh_shape = {
            'betas': (-1, 10),
            'transl': (-1, 3),
            'global_orient': (-1, 3),
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str,
        ...
        for cam_id in ['k1', 'k2', 'k3', 'k0']:
            with open(
                    os.path.join(dataset_path, 'calibs', 'intrinsics',
                                 cam_id[-1], 'calibration.json')) as f:
        ...
        for cam_id in ['k1', 'k2', 'k3', 'k0']:
            ...
            fx, fy = camera_intrinsics_dict[cam_id]['color'][
                'fx'], camera_intrinsics_dict[cam_id]['color']['fy']
            cx, cy = camera_intrinsics_dict[cam_id]['color'][
                'cx'], camera_intrinsics_dict[cam_id]['color']['cy']
            intrinsics[cam_id] = np.array(
                [[fx, 0, cx], [0, fy, cy], [0, 0, 1]], dtype=np.float32)
            dist_coeffs[cam_id] = np.array(
                camera_intrinsics_dict[cam_id]['color']['opencv'][4:])

            # get undistort camera_matrix
            intrinsics_undistorted[cam_id], _ = cv2.getOptimalNewCameraMatrix(
                intrinsics[cam_id],
                dist_coeffs[cam_id],
                self.misc_config['image_size'],
                alpha=0)
            # use transpose
        ...
        for seq in tqdm(
            ...
            valid_timestamp = [
                f for f in os.listdir(
                    os.path.join(dataset_path, 'sequences', seq))
                if f.endswith('.000')
            ]

            # get seq data
            info_file = os.path.join(dataset_path, 'sequences', seq,
                                     'info.json')
            ...
            for frame in tqdm(
                    valid_timestamp,
                    desc=f'Processing {seq} frames',
                    position=1,
                    leave=False):
                frame_data_folder = os.path.join(dataset_path, 'sequences',
                                                 seq, frame)

                # read smpl params
                smpl_param = np.load(
                    os.path.join(frame_data_folder, 'person', 'fit02',
                                 'person_fit.pkl'),
                    allow_pickle=True)

                # read joints3d
                j3d_p = os.path.join(frame_data_folder, 'person',
                                     'person_J3d.json')
                with open(j3d_p) as f:
                    joints3d = np.array(json.load(f)['body_joints3d']).reshape(
                        -1, 4)[:, :3]
                ...
                for view_id in sorted(kinect_view_ids):
                    ...
                    j2d_gt = json.load(
                        open(
                            os.path.join(frame_data_folder,
                                         view_id + '.color.json')))

                    for key in j2d_gt.keys():
                        j2d_gt[key] = np.array(j2d_gt[key]).reshape(-1,
                                                                    3)[:, :2]

                    # deal with kinect transform
                    # get kinect camera params
                    with open(
                            os.path.join(dataset_path, 'calibs', seq[:6],
                                         'config', view_id[-1],
                                         'config.json')) as f:
                        camera_param = json.load(f)
                    world2local_r = np.array(camera_param['rotation']).reshape(
                        3, 3)
                    world2local_t = np.array(
                        camera_param['translation']).reshape(3)
                    ...
                    # get smpl params
                    betas = np.array(smpl_param['betas'][:10]).reshape(
                        (-1, 10))
                    trans = np.array(smpl_param['trans']).reshape((-1, 3))
                    root_orient = np.array(smpl_param['pose'][:3]).reshape(
                        (-1, 3))
                    pose_body = np.array(smpl_param['pose'][3:66]).reshape(
                        (-1, 21, 3))
                    pose_hand = smpl_param['pose'][66:]
                    left_hand_pose = np.array(pose_hand[:45]).reshape(
                        -1, 15, 3)
                    right_hand_pose = np.array(pose_hand[45:]).reshape(
                        -1, 15, 3)

                    # get output kps3d
                    smpl_pose = np.concatenate([
                        pose_body,
                        np.array([0, 0, 0], dtype=np.float32).reshape(
                            -1, 1, 3),
                        np.array([0, 0, 0], dtype=np.float32).reshape(
                            -1, 1, 3)
                    ],
                    ...
                    output = gendered_model[gender](
                        ...
                        body_pose=torch.tensor(
                            smpl_pose.reshape(1, -1), device=self.device),
                        global_orient=torch.tensor(
                            global_orient_kinect.reshape(1, -1),
                            device=self.device),
                        transl=torch.tensor(
                            transl_kinect.reshape(1, -1), device=self.device),
                    )
                    keypoints_3d = output['joints']
                    keypoints_2d_xyd = cameras_opencv[
                        view_id].transform_points_screen(keypoints_3d)
                    ...
                    bbox_sacle_name = [
                        'bbox_body_scale', 'bbox_facehand_scale',
                        'bbox_facehand_scale', 'bbox_facehand_scale'
                    ]
                    for bbox_id, key in enumerate(sorted(j2d_gt.keys())):
                        bbox_xyxy = self._keypoints_to_scaled_bbox(
                            j2d_gt[key],
                    ...
                    img_undis_p = os.path.join(
                        os.path.join(frame_data_folder,
                                     view_id + '.color_undistorted.jpg'))
                    img = cv2.imread(
                        os.path.join(frame_data_folder,
                                     view_id + '.color.jpg'))
                    undistorted_img = cv2.undistort(img, intrinsics[view_id],
                                                    dist_coeffs[view_id])
                    if not os.path.exists(img_undis_p):
                        cv2.imwrite(img_undis_p, undistorted_img)

                    # save image path
                    image_path = img_undis_p.replace(
                        dataset_path + os.path.sep, '')
                    ...
                    j2d = np.concatenate([
                        j2d_gt['body_joints'], j2d_gt['left_hand_joints'],
                        j2d_gt['right_hand_joints'], j2d_gt['face_joints']
                    ],
                                         axis=0)
                    j2d_conf = np.zeros((137, 1))
                    j2d_conf[np.any(j2d != 0, axis=1)] = 1
                    keypoints2d_original_.append(
                        np.concatenate([j2d, j2d_conf], axis=1))
                    ...
                    for i in range(len(j2d)):
                        cv2.circle(undistorted_img,
                                   (int(j2d[i][0]), int(j2d[i][1])), 3,
                                   (0, 255, 0), -1)
                    cv2.imwrite(
                        os.path.join(out_path, f'{seq}_{frame}_{view_id}.jpg'),
                        undistorted_img)
        ...
        keypoints2d_original = np.concatenate(keypoints2d_original_).reshape(
            -1, 137, 3)
        ...
        keypoints3d_original = np.concatenate(keypoints3d_original_).reshape(
            -1, 25, 3)
        keypoints3d_ = np.ones([keypoints3d.shape[0], 25, 1])
        keypoints3d_original = np.concatenate(
            [keypoints3d_original, keypoints3d_], axis=-1)
        ...
        out_file = os.path.join(
            out_path,
            f'behave_{mode}_{seed}_{"{:03d}".format(size)}_downsampled.npz')
