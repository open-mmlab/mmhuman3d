...
class HumanartConverter(BaseModeConverter):
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str,
        ...
        modes_dict = {
            ...
            '2D_virtual_human': ['cartoon', 'digital_art', 'ink_painting', 'kids_drawing', 
        ...
        for part in batch_part:
            ...
            for name in batch_name:
                ...
                # load annotations
                anno = json.load(open(os.path.join(dataset_path, 'annotations', f'{part}_humanart_{name}.json')))
                
                size_n += len(anno['annotations'])

                # pdb.set_trace()

                # cerate image reference dict
                image_ref = {}
                for idx, img_dict in enumerate(anno['images']):
                    image_ref[img_dict['id']] = idx

                # iterate over images
                for idx, annotation in enumerate(tqdm(anno['annotations'], 
                    ...
                    # get image path
                    imgp = anno['images'][image_idx]['file_name'].split(os.path.sep, 1)[-1]
                    image_path_.append(imgp)

                    # get image size
                    height, width = anno['images'][image_idx]['height'], anno['images'][image_idx]['width'] 
                    ...
                    # get keypoints
                    keypoints21 = np.array(annotation['keypoints_21']).reshape(-1, 3)
                    ## change [x, y, 2] to [x, y, 1] (conf == 1)
                    keypoints21[:, -1] = (keypoints21[:, -1] == 2).astype(np.float32)
            ...
            # meta
            meta_ = {'image_height': height_, 'image_width': width_, 'image_description': description_,
            ...
            out_file = os.path.join(out_path, f'humanart_{mode}_{part}_{seed}_{"{:05d}".format(size_i)}.npz')
