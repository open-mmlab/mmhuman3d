...
class ShapyConverter(BaseModeConverter):
    ...
    def __init__(self, modes: List = []) -> None:
        ...
        self.misc_config = dict(
            ...
            hand_bbpx='by_dataset',
            bbox='by_dataset',
            keypoints2d='by_dataset',
        )
        self.smplx_shape = {
            'betas': (-1, 10),
            'transl': (-1, 3),
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str,
        ...
        kps_anno_ps = glob.glob(
            os.path.join(dataset_path, 'HBW', 'keypoints', f'{mode}', '**',
                         '*.json'),
            recursive=True)
        ...
        for kps_anno_p in tqdm(kps_anno_ps):
            ...
            # get image path
            image_p = kps_anno_p.replace('keypoints',
                                         'images').replace('.json', '.png')
            ...
            image_path = os.path.sep.join(
                image_p.split(os.path.sep)[root_idx + 1:])
            image_path_.append(image_path)

            # get keypoints
            kps2d_b = np.array(
                kps_anno['people'][0]['pose_keypoints_2d']).reshape(-1, 3)
            kps2d_lh = np.array(
                kps_anno['people'][0]['hand_left_keypoints_2d']).reshape(
                    -1, 3)
            kps2d_rh = np.array(
                kps_anno['people'][0]['hand_right_keypoints_2d']).reshape(
                    -1, 3)
            kps2d_f = np.array(
                kps_anno['people'][0]['face_keypoints_2d']).reshape(-1, 3)
            ...
            if mode == 'val':
                ...
                betas_param = dict(
                    np.load(
                        os.path.join(betas_path, f'{person_id}_param.npz'),
                        allow_pickle=True))['betas']
            ...
            bbox_xyxy = self._keypoints_to_scaled_bbox(
                kps2d_b[..., :2], scale=1.35)
            bbox_xywh = self._xyxy2xywh(bbox_xyxy)

            lhand_bbox_xyxy = self._keypoints_to_scaled_bbox(
                kps2d_lh[..., :2], scale=1.0)
            lhand_bbox_xywh = self._xyxy2xywh(lhand_bbox_xyxy)

            rhand_bbox_xyxy = self._keypoints_to_scaled_bbox(
                kps2d_rh[..., :2], scale=1.0)
            rhand_bbox_xywh = self._xyxy2xywh(rhand_bbox_xyxy)

            face_bbox_xyxy = self._keypoints_to_scaled_bbox(
                kps2d_f[..., :2], scale=1.0)
            face_bbox_xywh = self._xyxy2xywh(face_bbox_xyxy)

            for bbox in [
                    bbox_xywh, lhand_bbox_xywh, rhand_bbox_xywh, face_bbox_xywh
            ]:
                if bbox[2] * bbox[3] > 0:
                    bbox.append(1)
                else:
        ...
        human_data['lhand_bbox_xywh'] = np.array(lhand_bbox_xywh_).reshape(
            -1, 5)
        human_data['rhand_bbox_xywh'] = np.array(rhand_bbox_xywh_).reshape(
            -1, 5)
        ...
        out_file = os.path.join(out_path,
                                f'shapy_{mode}_{seed}_{str(size)}.npz')
