...
class BedlamConverter(BaseModeConverter):
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str,
        ...
        smplx = {}
        smplx_key = [
            'betas', 'betas_neutral', 'transl', 'global_orient', 'body_pose',
            'left_hand_pose', 'right_hand_pose', 'leye_pose', 'reye_pose',
            'jaw_pose', 'expression', 'flat_hand_mean'
        ]
        ...
        for j, instance_name in tqdm(enumerate(instance_names)):
            ...
            try:
                body_data = np.load(
                    os.path.join(data_folder, instance_name + '.npz'))
            except:
                continue
            ...
            global_orient = torch.FloatTensor(global_orient_).view(-1,
                                                                   3)  # (1,3)
            body_pose = torch.FloatTensor(body_pose_).view(-1,
                                                           21 * 3)  # (21,3)
            betas = torch.FloatTensor(betas_).view(-1,
                                                   11)  # SMPLX shape parameter
            transl = torch.FloatTensor(transl_).view(-1,
                                                     3)  # translation vector
            left_hand_pose = torch.FloatTensor(left_hand_pose_).view(
                -1, 15 * 3)
            right_hand_pose = torch.FloatTensor(right_hand_pose_).view(
                -1, 15 * 3)
            ...
            keypoints3d_smplx_img = cam2pixel(keypoints3d_smplx_cam, focal_,
                                              princpt_)
            ...
            for i, img_name in enumerate(img_names_):
                ...
                if 'closeup' in seq_folder:
                    ...
                    p_folder = os.path.dirname(
                        img_path.replace('_closeup', ''))
                    if not os.path.exists(p_folder):
                        os.makedirs(p_folder)

                    cv2.imwrite(
                        os.path.join(
                            seq_folder.replace('_closeup', ''), img_name), img)

                height = img.shape[0]
                width = img.shape[1]

                if False:
                    keypoints3d_smplx_cam = smplx_res['joints'].detach().cpu(
                    ).numpy()[0]
                    keypoints3d_smplx_img = cam2pixel(keypoints3d_smplx_cam,
                                                      focal_[i], princpt_[i])
                    keypoints2d_smplx = keypoints3d_smplx_img[:, :2]
                ...
                image_path_.append(
                    os.path.join(
                        instance_name.replace('_closeup', ''), 'png',
                        img_name))
                ...
                smplx['global_orient'].append(
                    np.array(global_orient[i]).reshape(3))
                smplx['body_pose'].append(
                    np.array(body_pose[i]).reshape(21, 3))
                smplx['transl'].append(np.array(transl[i]).reshape(3))
                smplx['left_hand_pose'].append(
                    np.array(left_hand_pose[i]).reshape(15, 3))
                smplx['right_hand_pose'].append(
                    np.array(right_hand_pose[i]).reshape(15, 3))
        ...
        human_data.dump(
            '/mnt/lustre/share_data/sunqingping/edpose/data/preprocessed_npz/bedlam_train_fix_img_shape.npz'
        )
