...
class Ssp3dConverter(BaseConverter):
    ...
    def __init__(self):
        ...
        self.misc = dict(bbox_body_scale=1.2, bbox_facehand_scale=1.0, bbox_source='keypoints2d_smpl',
                         cam_param_source='estimated', smpl_source='original', focal_length=5000, 
                         principal_point=(256, 256), image_size=512,)
        self.smpl_shape = {'betas': (-1, 10), 'transl': (-1, 3), 'global_orient': (-1, 3), 'body_pose': (-1, 23, 3)}
        

        super(Ssp3dConverter, self).__init__()

    
    def _keypoints_to_scaled_bbox_bfh(self, keypoints, vertices=None, occ=None, body_scale=1.0, fh_scale=1.0, convention='smpl_45'):
    ...
    def convert(self, dataset_path: str, out_path: str) -> dict:
        ...
        for idx in tqdm(range(len(anno['fnames']))):
            ...
            vertices_2d = camera.transform_points_screen(vertices)[..., :2].detach().cpu().numpy()
            ...
            bboxs = self._keypoints_to_scaled_bbox_bfh(keypoints_2d, vertices=vertices_2d, body_scale=self.misc['bbox_body_scale'],
                    fh_scale=self.misc['bbox_facehand_scale'], convention='smpl_vertices')
        ...
        # bboxs
        for bbox_name in ['bbox_xywh', 'face_bbox_xywh', 'lhand_bbox_xywh', 'rhand_bbox_xywh']:
            bboxs_[bbox_name] = np.concatenate(bboxs_[bbox_name], axis=0).reshape(-1, 5)
            human_data[bbox_name] = bboxs_[bbox_name]

        # keypoints 2d original
        keypoints2d_original = np.concatenate(keypoints2d_original_, axis=0).reshape(-1, 17, 3)
        ...
        out_file = os.path.join(out_path, f'ssp3d_{seed}_{"{:03d}".format(size)}.npz')
