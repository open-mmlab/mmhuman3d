...
class GTAHuman2Converter(BaseModeConverter):
    ...
    def _keypoints_to_scaled_bbox_fh(self,
        ...
        for body_part in ['head', 'left_hand', 'right_hand']:
            ...
            if np.sum(self_occ_p) / len(kp_id) >= 0.5 or np.sum(occ_p) / len(
                    kp_id) >= 0.5:
                conf = 0
    ...
    def _revert_smplx_hands_pca(self, param_dict, num_pca_comps):
        ...
        smplx_model = dict(
            np.load(
                'data/body_models/smplx/SMPLX_NEUTRAL.npz', allow_pickle=True))
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str,
        ...
        if mode == 'single':
            ann_paths_total = sorted(
                glob.glob(
                    os.path.join(dataset_path, 'annotations_single_smplx',
                                 '*.npz')))
        elif mode == 'multiple':
            ann_paths_total = sorted(
                glob.glob(
                    os.path.join(dataset_path, 'annotations_multiple_smplx',
                                 '*.npz')))

        print(
            f'{len(ann_paths_total)} sequences are avliable in this mode: {mode}'
        )
        ...
        for i in range(s_num):
            ...
            smplx = {}
            smplx['body_pose'], smplx['transl'], smplx['global_orient'], smplx[
                'betas'] = [], [], [], []
            ...
            for ann_path in tqdm(
                    ann_paths, desc=f'Processing {i}/{s_num} slices'):
                ...
                base = os.path.basename(
                    ann_path
                )  # -> seq_00090376_154131.npz -> seq_00090376_154131
                seq_idx, ped_idx = base[4:12], base[13:
                                                    19]  # -> 00090376, 154131
                ...
                if hand_pca_comps < 45:
                    ann = self._revert_smplx_hands_pca(
                        param_dict=ann, num_pca_comps=hand_pca_comps)
                    left_hand_pose = ann['left_hand_pose']
                    right_hand_pose = ann['right_hand_pose']
                ...
                for key in smplx_shape.keys():
                    if key in intersect_key:
                        body_model_param_tensor[key] = torch.tensor(
                            np.array(ann[key]).reshape(smplx_shape[key]),
                            device=self.device,
                            dtype=torch.float32)
                    else:
                ...
                output = body_model(
                    return_verts=True, **body_model_param_tensor)
                ...
                if np.sum(np.isnan(keypoints_2d)) + np.sum(
                        np.isnan(keypoints_3d)) > 0:
                ...
                for frame_idx in range(num_frames):
                    ...
                    image_path = os.path.join('images_' + mode,
                                              'seq_' + seq_idx,
                                              '{:08d}.jpeg'.format(frame_idx))
                    image_path_real = os.path.join(
                        f'/mnt/e/gta_human2/images_{mode}', 'seq_' + seq_idx,
                        '{:08d}.jpeg'.format(frame_idx))
            ...
            keypoints3d = np.concatenate(
                [keypoints3d,
                 np.ones([keypoints3d.shape[0], 144, 1])],
                axis=-1)
