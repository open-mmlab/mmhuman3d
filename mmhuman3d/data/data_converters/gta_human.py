...
def perspective_projection(
    ...
    projected_points = projected_points[:, :, :2] / projected_points[:, :, 2].unsqueeze(-1)  # (bs, N, 2)
...
class GTAHumanConverter(BaseConverter):
    """GTA-Human dataset
    `Playing for 3D Human Recovery' arXiv`2021
    ...
    def convert(self, dataset_path: str, out_path: str) -> dict:
        ...
        anno_paths = glob.glob(os.path.join(dataset_path, 'annotations', '*.pkl'))
        ...
        for anno_path in tqdm(anno_paths):
            ...
            base = os.path.basename(anno)  # -> seq_00000001.pkl
            seq_idx, _ = os.pathsplitext(base)  # -> seq_00000001
            num_frames = len(anno['body_pose'])

            # keypoints_2d_gta_openpose = map_joints(annotation['keypoints_2d'],
            ...
            keypoints_2d_gta, keypoints_2d_gta_mask = convert_kps(anno['keypoints_2d'],
                                                          src='gta', dst='smpl_49')
            keypoints_3d_gta, keypoints_3d_gta_mask = convert_kps(anno['keypoints_2d'],
                                                          src='gta', dst='smpl_49')
            ...
            keypoints_2d = perspective_projection(keypoints_3d, self.focal_length, self.camera_center)

            # root align
            root_idx = get_keypoint_idx('pelvis_extra', convention='smpl_49')
            keypoints_gta_3d = keypoints_gta_3d - keypoints_gta_3d[:, root_idx, :]
            ...
            for frame_idx in range(num_frames):
                ...
                # keypoints_2d_gta_openpose_frame = keypoints_2d_gta_openpose[frame_idx]
                # keypoints_2d_gta_joint24_frame = keypoints_2d_gta_joint24[frame_idx]
                # keypoints_3d_gta_joint24_frame = keypoints_3d_gta_joint24[frame_idx]
                #
                # keypoints_2d_smpl_openpose_frame = keypoints_2d_smpl_openpose[frame_idx]
                # keypoints_2d_smpl_joint24_frame = keypoints_2d_smpl_joint24[frame_idx]
                # keypoints_3d_smpl_joint24_frame = keypoints_3d_smpl_joint24[frame_idx]

                # # re-center 3d keypoints with the pelvis as the origin
                # keypoints_3d_gta_joint24_frame[:, :3] = keypoints_3d_gta_joint24_frame[:,
                #                                         :3] - keypoints_3d_gta_joint24_frame[14, :3]
                # keypoints_3d_smpl_joint24_frame[:, :3] = keypoints_3d_smpl_joint24_frame[:,
                #                                          :3] - keypoints_3d_smpl_joint24_frame[14, :3]
                ...
                global_orient.append(global_orient[frame_idx])
                body_pose.append(body_pose[frame_idx])
                betas.append(betas[frame_idx])
                transl.append(transl[frame_idx])

                keypoints_2d_gta.append(keypoints_2d_gta[frame_idx])
                keypoints_3d_gta.append(keypoints_3d_gta[frame_idx])
                keypoints_2d.append(keypoints_2d[frame_idx])
                keypoints_3d.append(keypoints_3d[frame_idx])
        ...
        keypoints2d_gta = np.array(keypoints_2d_gta_list).reshape(-1, 49, 2)
        keypoints2d_gta = np.concatenate([
            keypoints2d_gta,
            np.ones([keypoints2d_gta.shape[0], 49, 1])], axis=-1)
        ...
        keypoints3d_gta = np.array(keypoints_3d_gta_list).reshape(-1, 49, 3)
        keypoints3d_gta = np.concatenate([
            keypoints3d_gta,
            np.ones([keypoints3d_gta.shape[0], 49, 1])], axis=-1)
