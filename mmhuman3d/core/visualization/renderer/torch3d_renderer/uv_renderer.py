...
class UVRenderer(nn.Module):
        ...
        if param_path is not None:
            ...
            if resolution != (256, 256):
                self.bary_coords = F.interpolate(
                    self.bary_coords[None], resolution, mode='bicubic').clamp(
                        min=0, max=1)[0]
                self.pix_to_face = F.interpolate(
                    self.pix_to_face[None], resolution,
                    mode='bicubic').long()[0]
    ...
    def forward(self, verts_attr: Optional[torch.Tensor]) -> torch.Tensor:
        """[summary]
    ...
    def forward_uvd_map(self,
        ...
        uvd_map = self.forward(verts_attr=verts_uvd)
        return uvd_map
