...
def init_model(config, checkpoint=None, device='cuda:0'):
    ...
    if config.model.type == 'VideoBodyModelEstimator':
        extrator = build_backbone(config.extrator.backbone)
        if config.extrator.checkpoint is not None:
            # load model checkpoint
            load_checkpoint(extrator, 'data/vibe_backbone.pth')
        extrator.cfg = config
        extrator.to(device)
        extrator.eval()
    return model, extrator
...
def inference_image_based_model(
    ...
    for i, bbox in enumerate(bboxes_xywh):
        x, y, w, h, _ = bbox
        center, scale = box2cs(x, y, w, h, aspect_ratio, scale_mult=1.2)
        # prepare data
        data = {
...
def feature_extract(
    ...
    for i, bbox in enumerate(bboxes_xywh):
        ...
        center, scale = box2cs(x, y, w, h, aspect_ratio, scale_mult=1)
    ...
    for idx in range(len(det_results)):
        ...
        feature_result['features'] = results[idx]
        feature_results.append(feature_result)

    return feature_results


def _gather_input_features(det_results):
    """Gather input features.

    Args:
        pose_results (List[List[Dict]]): Multi-frame feature extraction results
    ...
    for idx in range(N):
        ...
        if not with_track_id:
            feature_seq['features'] = np.stack([
                frame[idx]['features'].cpu().numpy()
                for frame in extracted_features
            ])
        else:
            ...
            for frame_idx in range(target_frame - 1, -1, -1):
                ...
                for res in extracted_features[frame_idx]:
                    if res['track_id'] == track_ids[idx]:
                        features[frame_idx] = res['features'].cpu().numpy()
                        contains_idx = True
                        break
                if not contains_idx:
...
def inference_video_based_model(
    ...
    Args:
        ...
        det_results ([type]): [description]
        with_track_id (bool, optional): [description]. Defaults to True.
    ...
    target_idx = 0 if True else len(det_results) // 2

    input_features = _gather_input_features(det_results)
