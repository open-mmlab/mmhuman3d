...
def test_smpl():
    ...
    assert torch.isclose(smpl_54_joints[:, joint_mapping, :], smpl_49_joints).all()
...
def test_gendered_smpl():
    ...
    smpl_neutral = build_body_model(dict(
        type='SMPL',
        gender='neutral',
    ...
    smpl_male = build_body_model(dict(
        type='SMPL',
        gender='male',
        keypoint_src='smpl_45',
        keypoint_dst='smpl_45',
    ...
    smpl_female = build_body_model(dict(
        type='SMPL',
        gender='female',
        keypoint_src='smpl_45',
        keypoint_dst='smpl_45',
    ...
    gendered_smpl = build_body_model(dict(
        type='GenderedSMPL',
        keypoint_src='smpl_45',
        keypoint_dst='smpl_45',
        model_path=body_model_load_dir
    ...
    betas_concat = torch.cat([random_betas_neutral, random_betas_male, random_betas_female])
    joint_concat = torch.cat([smpl_neutral_output['joints'], smpl_male_output['joints'], smpl_female_output['joints']])

    gendered_smpl_output = gendered_smpl(
        betas=betas_concat,
        gender=gender
    )
