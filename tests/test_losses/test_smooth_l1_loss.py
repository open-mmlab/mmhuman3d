...
def test_l1_loss():
    ...
    # test reduction
    loss_cfg = dict(type='L1Loss', reduction='sum')
    loss = build_loss(loss_cfg)
    ...
    fake_pred = torch.ones((1, 3, 2))
    fake_target = torch.zeros((1, 3, 2))
    assert torch.allclose(loss(fake_pred, fake_target), torch.tensor(6.))

    # test loss weight
    loss_cfg = dict(type='L1Loss', loss_weight=2.0)
    loss = build_loss(loss_cfg)
    ...
    fake_pred = torch.ones((1, 3, 2))
    fake_target = torch.zeros((1, 3, 2))
    assert torch.allclose(loss(fake_pred, fake_target), torch.tensor(2.))
